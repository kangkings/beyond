...
import com.example.common.queries.GetProductByProductIdQuery;
...
import com.example.orderservice.command.UpdateOrderCommand;
...
public class OrderService {
    ...
    */
    public ResultVO<CreateOrderCommand> createOrder(OrderReqCreateDTO orderReqCreateDTO) {
        ...
        log.info("===== [Create Order] #1: <isValidProduct> =====");
        List<ResultVO<ProductDTO>> products = getProduct(orderReqCreateDTO.getOrderReqDetails());
        String retCheck = isValidProduct(products);
        if (!retCheck.isEmpty()) {
        ...
        List<OrderDetailDTO> newOrderDetails = orderReqCreateDTO.getOrderReqDetails().stream()
                .map(o -> new OrderDetailDTO(orderId, o.getProductId(), o.getQty(), o.getQty() * getUnitPrice(products, o.getProductId())))
    ...
    */
    public ResultVO<UpdateOrderCommand> updateOrder(OrderReqUpdateDTO orderReqUpdateDTO) {
        ...
        log.info("===== [Update Order] #2: <isValidProduct> =====");
        List<ResultVO<ProductDTO>> products = getProduct(orderReqUpdateDTO.getOrderReqDetails());
        String strCheck = isValidProduct(products);
        if (!strCheck.isEmpty()) {
            retVo.setReturnCode(false);
        ...
        for(OrderDetail item:order.getOrderDetails()) {
            ...
            if(optObj.isPresent()) {    //변경된 주문 상세 정보 반영
                ...
                newOrderDetail = new OrderDetailDTO(orderId, reqDetail.getProductId(),
                        reqDetail.getQty(), reqDetail.getQty() * getUnitPrice(products, reqDetail.getProductId()));
    ...
    */
    ...
    private List<ResultVO<ProductDTO>> getProduct(List<OrderReqDetailDTO> orderDetails) {
        log.info("[OrderService] Executing getProduct");

        GetProductByProductIdQuery getProductByProductIdQuery;
        List<ResultVO<ProductDTO>> products = new ArrayList<>();
        ResultVO<ProductDTO> retVo;
        int reqQty;
        ...
        try {
            for (OrderReqDetailDTO orderDetail : orderDetails) {
                ...
                retVo = new ResultVO<>();
                retVo.setResult(productDTO);
                retVo.setReturnCode(reqQty <= productDTO.getProductQty() && productDTO.getProductQty() != 0);
                products.add(retVo);
            }
        } catch (Exception e) {
    ...
    //-- 상품 재고가 있는지 검사하여 리턴
    private String isValidProduct(List<ResultVO<ProductDTO>> products) {
        log.info("[OrderService] Executing <isValidProduct>");

        if(products.isEmpty()) return "재고 정보 없음";

        for (ResultVO<ProductDTO> retVo : products) {
    ...
    //-- 제품ID에 해당하는 제품단가를 리턴
    private int getUnitPrice(List<ResultVO<ProductDTO>> products, String productId) {
        log.info("[OrderService] Executing <getUnitPrice>");

        Optional<ResultVO<ProductDTO>> optProduct = products.stream()
                .filter(obj -> productId.equals(obj.getResult().getProductId()))
                .findFirst();
        return (optProduct.map(productDTOResultVO -> productDTOResultVO.getResult().getUnitPrice()).orElse(0));
    }
